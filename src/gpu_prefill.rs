...
impl PrefillEngine {
        ...
        for chunk_idx in 0..num_chunks {
            ...
            for layer_idx in 0..num_hidden_layers {
                ...
                match layer_type {
                    ...
                    3 => self
                        .forward_linear_attention(layer_idx, m)
            ...
            match (
                ...
                (Err(err), _) | (_, Err(err)) => {
                    trace_emit_prefill_mark(
                        ...
                        "prefill",
                        &format!(
                            "phase=final_prefill_trace_output_download_failed error={}",
                            err
                        ),
                    );
                }
                ...
                *self.scratch.d_hidden.device_ptr() + (last_pos * h * 2) as u64,
                ...
                Err(err) => {
                    trace_emit_prefill_mark(
                        ...
                        "prefill",
                        &format!(
                            "phase=final_hidden_for_logits_download_failed error={}",
                            err
                        ),
                    );
                }
    ...
    fn forward_gqa_chunked(
        ...
        if let Some(fa2_fwd) = self.kernels.flash_attn_fwd {
                ...
                && self.kv_k_radius_ptrs[layer_idx] != 0
            {
                ...
                let use_fp8_fa2 =
                    self.kernels.flash_attn_fwd_fp8kv.is_some() && cfg.head_dim <= 128;
    ...
    fn forward_moe_fused(&mut self, layer_idx: usize, m: usize) -> Result<(), String> {
        ...
        if moe_trace_enabled {
            ...
            if m_topk <= 4096 {
                        ...
                        match isolated_scatter_result {
                            ...
                            Err(err) => {
                                trace_emit_prefill_mark(
                                    ...
                                    "prefill_layer",
                                    &format!(
                                        "phase=moe_routed_accum_isolated_scatter_failed error={}",
                                        err
                                    ),
                                );
                            }
                        ...
                        match vendor_scatter_replay_result {
                            ...
                            Err(err) => {
                                trace_emit_prefill_mark(
                                    ...
                                    "prefill_layer",
                                    &format!(
                                        "phase=moe_routed_accum_vendor_scatter_replay_failed error={}",
                                        err
                                    ),
                                );
                            }
                        ...
                        match download_device_i32(
                            ...
                            Err(err) => {
                                trace_emit_prefill_mark(
                                    ...
                                    "prefill_layer",
                                    &format!(
                                        "phase=moe_last_token_row_download_failed error={}",
                                        err
                                    ),
                                );
                            }
                ...
                trace_emit_prefill_mark(
                    ...
                    "prefill_layer",
                    &format!(
                        "phase=moe_routed_accum_trace_skipped total_active={}",
                        m_topk
                    ),
                );
            }
        ...
        if has_shared {
            ...
            if shared_add_on_shared_stream {
                unsafe {
                    cuda_sys::lib().cuEventRecord(self.shared_event, self.shared_stream);
                    cuda_sys::lib().cuStreamWaitEvent(self.stream, self.shared_event, 0);
                }
                            ...
                            match isolated_shared_add_result {
                                ...
                                Err(err) => {
                                    trace_emit_prefill_mark(
                                        ...
                                        "prefill_layer",
                                        &format!(
                                            "phase=moe_shared_add_isolated_failed error={}",
                                            err
                                        ),
                                    );
                                }
                            ...
                            match isolated_shared_add_full_result {
                                ...
                                Err(err) => {
                                    trace_emit_prefill_mark(
                                        ...
                                        "prefill_layer",
                                        &format!(
                                            "phase=moe_shared_add_isolated_full_batch_failed error={}",
                                            err
                                        ),
                                    );
                                }
    ...
    fn forward_moe_sequential(&mut self, layer_idx: usize, m: usize) -> Result<(), String> {
        ...
        let shared_async =
            has_shared && all_cached && !(diag && Self::diag_layer_enabled(layer_idx));
        if shared_async {
            unsafe {
                cuda_sys::lib().cuEventRecord(self.compute_event, self.stream);
...
mod kernel_tests {
    ...
    fn test_gqa_cross_chunk_attention() {
        ...
        unsafe {
            cuda_sys::lib().cuMemcpyHtoD_v2(
                *d_k_cache.device_ptr(),
                k_fp8_data.as_ptr() as *const _,
                k_fp8_data.len(),
            );
            cuda_sys::lib().cuMemcpyHtoD_v2(
                *d_v_cache.device_ptr(),
                v_fp8_data.as_ptr() as *const _,
                v_fp8_data.len(),
            );
        }
    ...
    fn polar4_threads(num_blocks: usize) -> u32 {
    ...
    fn test_polar4_kv_roundtrip_smoke() {
        ...
        assert!(
            k_max < 0.40 && v_max < 0.40,
            "polar4 roundtrip too lossy: k_max={k_max:.6} v_max={v_max:.6}"
        );
        assert!(
            k_avg < 0.10 && v_avg < 0.10,
            "polar4 roundtrip avg too lossy: k_avg={k_avg:.6} v_avg={v_avg:.6}"
        );
    }
    ...
    fn test_polar4_kv_append_bounds_smoke() {
        ...
        let check_u16_guards = |name: &str, data: &[u16]| {
            assert!(
                data[..guard_bf16].iter().all(|&x| x == radius_guard_val),
                "{name}: prefix guard overwritten"
            );
            assert!(
                data[guard_bf16 + radius_elems..]
                    .iter()
                    .all(|&x| x == radius_guard_val),
                "{name}: suffix guard overwritten"
            );
        };
        let check_u8_guards = |name: &str, data: &[u8]| {
            assert!(
                data[..guard_u8].iter().all(|&x| x == angle_guard_val),
                "{name}: prefix guard overwritten"
            );
            assert!(
                data[guard_u8 + angle_elems..]
                    .iter()
                    .all(|&x| x == angle_guard_val),
                "{name}: suffix guard overwritten"
            );
        };
