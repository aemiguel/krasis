...
import os as _os
_PROMPTS_DIR = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))), "benchmarks", "prompts")
_PROMPT_FILES = sorted(f for f in _os.listdir(_PROMPTS_DIR) if f.endswith(".txt"))
LONG_PROMPT = open(_os.path.join(_PROMPTS_DIR, _PROMPT_FILES[0])).read()[:1350]
