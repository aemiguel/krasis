...
def mamba_chunk_scan_combined(
    ...
    """
    Parallel chunked SSD scan for Mamba2 prefill.
    ...
    Returns:
        If return_final_states=True: (scan_output, final_states)
            scan_output: (batch, seqlen, nheads, headdim)
            final_states: (batch, nheads, headdim, dstate)
        If return_final_states=False: scan_output
    """
    ...
    # Pre-allocate output if not provided (needed for z=None case where
    # _chunk_scan_fwd writes directly to out)
    if out is None:
    ...
    # scan_output: when z is not None, out_x has pre-gated output and out has z-gated.
    # When z is None, out has the direct scan output and out_x is None.
    scan_output = out
    ...
    if not return_varlen_states:
        if not return_final_states:
            ...
            return scan_output, final_states
    else:
        varlen_states = rest[0]
        if not return_final_states:
            return scan_output, varlen_states
        else:
            return scan_output, final_states, varlen_states
